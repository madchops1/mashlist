 ...
 */ 
  ...
  // Chris Bolt - renamed wpro_xajax for compatibility with apps that alread have xajax in use - 18 Jan 08
class wpro_xajax
{
	/**#@+
	 * @access protected
	 ...
	 */
	function wpro_xajax($sRequestURI="",$sWrapperPrefix="xajax_",$sEncoding=XAJAX_DEFAULT_CHAR_ENCODING,$bDebug=false)
	{
	...
	{	
		...
		if ($this->bErrorHandler) {
			...
			// function name prefixed with wpro_ by chirs bolt march 2008.
			set_error_handler("wpro_xajaxErrorHandler");
		...
		if ($this->sPreFunction) {
			if (!$this->_isFunctionCallable($this->sPreFunction)) {
				...
				$oResponse = new wpro_xajaxResponse();
		...
		if ($bFoundFunction)
		{
			...
			if (!$bEndRequest) {
				...
				// Chris Bolt, bug fix: http://wiki.xajaxproject.org/Fix_Bugs_%28xajax_0.2.5%29
				...
				if ((!is_object($oResponse)) ||
                     (strtolower(get_class($oResponse)) <> "wpro_xajaxresponse") ||
					...
					$oNewResponse = new wpro_xajaxResponse($this->sEncoding, $this->bOutputEntities);
		...
		if ($this->bErrorHandler && !empty( $GLOBALS['xajaxErrorHandlerText'] )) {
			$oErrorResponse = new wpro_xajaxResponse();
	...
	function _detectURI() {
		...
		if (empty($aURL['path'])) {
			if (!empty($_SERVER['PATH_INFO'])) {
				...
				// Chris Bolt, changed PHP_SELF to SCRIPT_NAME
				//$sPath = parse_url($_SERVER['PHP_SELF']);
				$sPath = parse_url($_SERVER['SCRIPT_NAME']);
			}
			$aURL['path'] = str_replace(array('"',"'",'<','>'), array('%22','%27','%3C','%3E'), $sPath['path']);
			unset($sPath);
		}
		
		// chris bolt, added query string detection
		if (empty($aURL['query'])) {
		...
		// Chris Bolt, wrapped query addition in if empty statement
...
function wpro_xajaxErrorHandler($errno, $errstr, $errfile, $errline)
{

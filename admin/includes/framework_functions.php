 ...
 * 	Karl Steltenpohl Development LLC. All Rights Reserved.
 *
 *	
...
/**
 * NEW FUNCTION FOR CONVERTING A DATE AND TIME INTO SECONDS (UNIX TIMESTAMP)
 * 
 * 
 */
function DateTimeAmPmIntoSeconds($datetime_ampm)
{
	
	
	
	if($datetime_ampm == '//' || $datetime_ampm == ''){ return ""; }
	
	echo "<br><br>".$datetime_ampm."<br><br>";
	
	// GET DATE PARTS;
	// 02/25/2012 05:34 pm
	$date_parts_array = explode(" ",$datetime_ampm);
	$date_array = explode("/",$date_parts_array[0]);
	$time_array = explode(":",$date_parts_array[1]);
	...
	//echo " ".$month."/".$day."/".$year." ".$hour.":".$minute.":00 <br>";
	//$seconds = strtotime("".$year."-".$month."-".$day." ".$hour.":".$minute.":00 ".$ampm."");
	//$seconds = strtotime("".$year."-".$month."-".$day." ".$hour.":".$minute.":00");
	//$seconds = strtotime("".$month."/".$day."/".$year." ".$hour.":".$minute.":00");
	$seconds = strtotime("".$month."/".$day."/".$year." ".$hour.":".$minute.":00 ".$ampm."");
...
/**
 * NEW FUNCTION FOR CONVERTING SECONDS (UNIX TIMESTAMP) INTO HUMAN READABLE DATE
 */
function SecondsIntoDateTimeAmPm($seconds_ampm){
	
	// 02/25/2012 05:34 pm
	$date = date('m/d/Y h:i a', $seconds_ampm);
...
function wes_now(){
	$now == strtotime('now');
	// SELECT THE CURRENT OFFSET FROM THE 
	return $now;
}
 ...
 * This function checks if any rows in the table are scheduled for publishing now
 */
function publishPendingItems($table) 
{
	$now = strtotime('now');
	//$now = wes_now();
	$select = 	"SELECT * FROM ".$table." ".
...
/**
 * EXPIRE PUBLISHED ITEMS
 * This function checks if any rows in the table are scheduled for expiring now
 */
function expirePublishedItems($table){
	$now = strtotime('now');
	//$now = wes_now();
	$select =	"UPDATE ".$table." SET ".
				"status = 'Expired' ".

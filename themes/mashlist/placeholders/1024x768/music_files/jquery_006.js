...
(function($, undefined) {
	...
	$.jPlayer.prototype = {
		...
		options: { // Instanced in $.jPlayer() constructor
            ...
            solution: "html", // "html, flash" Valid solutions: html, flash. Order defines priority. 1st is highest,
			supplied: "mp3", // Defines which formats jPlayer will try and support and the priority by the order. 1st is highest,
			preload: 'metadata',  // HTML5 Spec values: none, metadata, auto.
			volume: 0.5, // The volume. Number 0 to 1.
			muted: false,
		...
		_updateInterface: function() {
			...
			if(this.css.jq.playBar.length) {
				if ($(".jp-play-bar").hasClass("ui-draggable-dragging")) {
					$(".jp-play-bar").css("left", $(".jp-play-bar").css("left"));
				} else {
					this.css.jq.playBar.css("left", this.status.currentPercentRelative+"%");
				}
			}
			if(this.css.jq.currentTime.length) {
		...
		volumeBarValue: function(v) { // Handles clicks on the volumeBarValue
			if(this.css.jq.volumeBarValue.length) {
				this.css.jq.volumeBarValue.css("left", (v*100)+"%");
			}
		},

...
function wproDialog () {
	...
	// generic function for writing preview frames
	...
	/* this function will always require an editor object */
	this.selectCurrentStyle = function (elm) {
		if (!elm.nodeType) return;
		if (currentEditor) {
			// go throug the element options and pull tghe values out into an array
	...
	/* functions copied from the wp object */
	...
	/* end functions copied from the wp object */
	...
	this.init = function () {
		...
		// first check if we are in an iframe or in a dialog...
		// in an iframe
		if (wproIframeDialogs==true) {
			...
			//this.events.addEvent(window, 'load', wproOnLoad);
		} else {
		...
		if (this.iframeDialog) {
			...
			if (WPro) {
				if (this.isIE) {
					// record selection incase it is lost through re-focusing
		...
		//this.locationRegex = new RegExp('(data=|href=|src=|action=)"' + WPro.quoteMeta(obj._location.replace(/&/gi,'&amp;'))+'#', 'gi');
...
function wp_load() {
	...
	/* fixes some rendering bugs in IE 6 */

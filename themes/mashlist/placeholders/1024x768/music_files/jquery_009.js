            ...
            if ( event.target !== editableElement[0] ) {
                switch ( event.type ) {
                    case 'dblclick':
                        ...
                        var elId = widget.element[0].getAttribute('id');
                        playlistUI.currentElId = elId;
...
$.inlineEdit.defaults = {
    ...
    save: function(ev, el) {
        // CUSTOM
        // playlistUI.inlineSaveNewMyPlaylist(ev, el);
    },
...
$.inlineEdit.prototype = {
    ...
    placeholderHtml: function(elem) {
        $el = $(elem);
        if ($el.data('default')) {
            return '<span class="default '+ placeholderClass +'" data-default="true">'+ $el.data('default') +'</span>';
        } else {
            return '<span class="default '+ placeholderClass +'">'+ this.options.placeholder +'</span>';
        }
    },
    ...
    change: function( elem, event ) {
        ...
        this.timer = window.setTimeout( function() {
            self.element.html( self.value() || self.placeholderHtml(elem) );

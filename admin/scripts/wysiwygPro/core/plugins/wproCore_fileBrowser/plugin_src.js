...
wproPlugin_wproCore_fileBrowser.prototype.init=function(EDITOR){
	...
	//EDITOR.addHTMLFilter('rawSource', wproPlugin_wproCore_fileBrowser_df);
	EDITOR.addHTMLFilter('source', wproPlugin_wproCore_fileBrowser_sf);
	//EDITOR.addHTMLFilter('preview', wproPlugin_wproCore_fileBrowser_sf);
...
wproPlugin_wproCore_fileBrowser.prototype._getArrAttributes = function (o, decode) {
	...
	if (attrs) {
		...
		for (var j=0; j < rl2; j++) {
			...
			//if (value=='') if (!allowedEmpty.test(name)) continue;
...
// converts the media string into the media data array
...
// converts object and embed HTML into the media data array
...
// converts the media data array into the media string
...
// converts the image placeholders into object tags
...
// converts object tags into image placeholders

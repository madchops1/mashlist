...
function ThreeSixtyPlayer() {
  ...
  this.events = {
    ...
    finish: function() {
	  ...
	  //trackId = $.cookies.get( 'trackId' );
	  mixId = $.cookies.get( 'mixId' );
	  var that = this;
      // LOAD NEXT SONG
      $.ajax({
			  url: "index.php",
			  ...
			  data: {'ajax': '1', 'action':'curl', 'curlurl':'http://8tracks.com/sets/'+token+'/next?mix_id='+mixId+''},
			  success: function(response5){
				  //console.log("NEXT SONG SUCCESS");
				  responseSet5 = $.parseJSON(response5);
				  //console.log(responseSet5);

				  var nextSongURL = responseSet5.set.track.url;
			      var nextSongName = responseSet5.set.track.name;
			  	  var nextTrackId = responseSet5.set.track.id;
			  	  $.cookies.set('trackId',nextTrackId);
			  	  $("#song-link").attr("href",nextSongURL);
			  	  $("#song-link").html(nextSongName);
			  	  $(".sm2-360btn").delay(300).trigger('click');
			  	  
			  	
				  //that.play('ui360Sound'+(self.soundCount++),nextSongURL);
			      console.log('URL: ' + nextSongName + ' ' + nextSongURL);
			  }
		});
  ...
  this.updatePlaying = function() {
    ...
    if (timeNow !== this._360data.lastTime) {
       ...
       */
      //console.log(timeNow);
      //$("#playlists").css("background-color","yellow");
      if(timeNow == "0:03"){
      ...
      if(timeNow == "0:30"){
    	  ...
    	  // curl http://8tracks.com/sets/874076615/report.xml?track_id=[track_id]&mix_id=[mix_id]
			$.ajax({
				  url: "index.php",
				  context: document.body,
				  type: "POST",
				  //data: {'ajax': '1', 'action':'curl', 'curlurl':'http://8tracks.com/sets/new?'},
				  data: {'ajax': '1', 'action':'curl', 'curlurl':'http://8tracks.com/sets/'+token+'/report.xml?track_id=['+trackId+']&mix_id=['+mixId+']'},
				  success: function(response4){
					  console.log("30 SEC SUCCESS CALLBACK FOR ONE PERFORMANCE");

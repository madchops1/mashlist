...
$BACKUP_DEST = '/home/ksdwesksd/public_html/backups/backups';
$BACKUP_TEMP = '/home/ksdwesksd/public_html/backups/backups_tmp';
$VERBOSE = true;
$BACKUP_NAME = 'ksdvd1_database_wes-shebeads_mysql_backup_' . date('Y-m-d');
$LOG_FILE = $BACKUP_NAME . '.log';
$ERR_FILE = $BACKUP_NAME . '.err';
$COMPRESSOR = 'bzip2';
$EMAIL_BACKUP = true;
$DEL_AFTER = false;
$EMAIL_FROM = 'KSDVD1 Server';
$EMAIL_SUBJECT = 'SQL Backup for KSDVD1 wes-shebeads Database ' . date('Y-m-d') . ' at ' . date('H:i');
$EMAIL_ADDR = 'karl@webksd.com';
$ERROR_EMAIL = 'karl@webksd.com';
$ERROR_SUBJECT = 'KSDVD1 SQL wes-shebeads BACKUP ERROR: ' . $EMAIL_SUBJECT;
$EXCLUDE_DB = 'information_schema,wes-demo,wes-infinitespanish,wes-karlsteltenpohl,wes-mecis,wes-repository,wes-shebeads-dev,wes-test,wes-v15,wes-webksd,wes-wescms';
...
//$current_path = $_SERVER['DOCUMENT_ROOT']'/backups_script';
//die($_SERVER['DOCUMENT_ROOT']);
//exit;
//if( file_exists( $current_path.'/database_backup_config.php' ) ) {
if( file_exists( 'shebeads_database_backup_config.php' ) ) {
	//require( $current_path.'/database_backup_config.php' );
	require( 'shebeads_database_backup_config.php' );
} else {
	echo 'No configuration file [shebeads_database_backup_config.php] found. Please check your installation.';
	exit;
}

...
wproPlugin_wproCore_fullWindow.prototype.getFullWindowButtons = function (view, EDITOR) {
	...
	for (var i = 0; i < n; i++) {
		...
		if (buttons[i].getAttribute("_wp_cid")=='fullwindow') {
			//EDITOR._designFullWindowButton = buttons[i];
...
function wproE_fullWindow () {
	...
	// hide select elements since they tend to show through in some browsers*/
	...
	//this.editorborder.style.top = document.body.scrollTop + document.documentElement.scrollTop + 'px';
	//this.editorborder.style.left = document.body.scrollLeft + document.documentElement.scrollLeft + 'px';
	...
	// calculate size of display above and display below.

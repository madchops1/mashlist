	...
	switch (FB.action) {
		...
		default:
			if (this.properties) {
				...
				// attempt to determine the correct plugin
				var i = 0;
				var other_index;
				var found = false;
				for (var x in this.remoteEmbedPrefixes) {
					// record other plugin for later
					if (x=='zz_other') {
						other_index = i;
					}
				...
				// use other plugin for unknown file types
				if (!found) {
...
fbObj.prototype.showContextMenu = function (e) {
	...
	// check for selected items and display open or insert at top of menu
	var forms = new wproForms();
	var form = document.dialogForm
...
fbObj.prototype.onLoadFolder = function (history) {
	...
	// scan for selected files and folders and select them, and scroll to them
...
fbObj.prototype.showMove = function (b) {
	...
	// check the permissions required to move the selected items (this affects interface only)
	var fp = '';
	var op = '';
	var forms = new wproForms();
	var folders = false;
...
fbObj.prototype.renameAction = function () {
	...
	//var form2 = parentWindow.document.dialogForm;
	var sortBy = form.sortBy.value
	var sortDir = form.sortDir.value
	var page = form.page.value
	var id = form.folderID.value
...
fbObj.prototype.selectFile = function (node, details, multiple, scroll) {
	...
	if (fileLen + foldLen==0) {	
		...
		//document.getElementById('displayFolderModified').innerHTML = '';
		//document.getElementById('displayFolderModified').appendChild(t);
		...
		if (details.name) {
			...
			if (details.prev) {
				//this.preview('filePanePreview', folderURL + details.name);
				//dialog.changeFrameLocation(document.getElementById('filePanePreview'), folderURL + details.name);
		...
		// additional details...
		if (this.action!='link'&&this.action!='document') this.awaitingFileDetails = true;//form.ok.disabled = true;
